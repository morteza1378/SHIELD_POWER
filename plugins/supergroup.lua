...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '<b>Done !</b>\n<code>This SuperGroup</code> <b>['..msg.to.print_name..'] </b><code>Has Been Added </code> <b>By </b>@'..msg.from.username..''
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '<b>Done !</b>\n<code>This SuperGroup</code> <b>['..msg.to.print_name..'] </b><code>Has Been Removed</code> <b>By </b>@'..msg.from.username..''
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function callbackres(extra, success, result)
  ...
  elseif get_cmd == "invite" then
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			unmute_user(chat_id, user_id)
			send_large_msg(receiver, '<b> ['..user_id..'] removed from muted user list</b>')
...
local function run(msg, matches)
	...
	local name_log = print_name:gsub("_", " ")
	local data = load_data(_config.moderation.data)
		if matches[1] == 'add'or matches[1]=='نصب' or matches[1]=='+' and not matches[2] then
			if not is_admin1(msg) and not is_support(support_id) then
				return
			end
			if is_super_group(msg) then
				return reply_msg(msg.id, '<code>SuperGroup</code>\n<code>Name :</code> <b>'..msg.to.print_name..' </b>\n<code>Id :</code> <b>'..msg.to.id..' </b>\n<code>Is Already Added </code> ', ok_cb, false)
			end
		...
		if matches[1] == 'rem' and is_admin1(msg) or matches[1]== 'حذف' and is_admin1(msg) or matches[1]== '-' and is_admin1(msg) and not matches[2] then
		...
		if matches[1] == 'link' or matches[1] =='لینک' then
            ...
            return reply_msg(msg.id,'➖➖➖➖➖➖➖➖➖➖➖➖➖➖\n<b>Group Link For</b> <code>'..msg.to.title..'</code>\n'..group_link..'\n➖➖➖➖➖➖➖➖➖➖➖➖➖➖', ok_cb, false)
		end
		...
		if matches[1] == 'mute' or matches[1] =='ممنوعیت' and is_owner(msg) then
			local chat_id = msg.to.id
		...
		if matches[1] == 'unmute' or matches[1] =='حذف ممنوعیت' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'audio' or matches[2] =='ویس' then
			local msg_type = 'Audio'
